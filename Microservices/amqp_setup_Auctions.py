...
def create_queues(channel):
    ...
    create_auction_bid_queue(channel)
    create_record_bid_queue(channel)

# function to create Auction_Bid queue  
def create_auction_bid_queue(channel):
    print('amqp_setup:create_auction_bid_queue')
    ...
    channel.queue_bind(exchange=exchangename, queue=a_queue_name, routing_key='*.auction')
        # bind the queue to the exchange via the key
        # 'routing_key=#' => any routing_key would be matched

# function to create Record_Bid queue  
def create_record_bid_queue(channel):
    print('amqp_setup:create_record_bid_queue')
    a_queue_name = 'Record_Bid'
    channel.queue_declare(queue=a_queue_name, durable=True) # 'durable' makes the queue survive broker restarts
    channel.queue_bind(exchange=exchangename, queue=a_queue_name, routing_key='*.record')
        # bind the queue to the exchange via the key
        # 'routing_key=#' => any routing_key would be matched
